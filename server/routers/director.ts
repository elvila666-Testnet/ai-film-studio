...
import { scenes, shots, projectContent, generations, shotActors, actors, audioAssets, type Scene, type Shot, type Generation, type StoryboardImage } from "../../drizzle/schema";
import { eq, inArray, desc, and } from "drizzle-orm";

export const directorRouter = router({
    generateCreativeProposal: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const contentResult = await db.select().from(projectContent).where(eq(projectContent.projectId, input.projectId)).limit(1);
            const content = contentResult[0];

            if (!content || (content as any).scriptStatus !== "approved") {
                throw new Error("Script must be approved before generating a creative proposal.");
            }

            const { projects } = await import("../../drizzle/schema");
            const projectResult = await db.select().from(projects).where(eq(projects.id, input.projectId)).limit(1);
            const project = projectResult[0];
            const { getBrandContext } = await import("../services/brandService");
            const brandDNA = project?.brandId ? await getBrandContext(project.brandId) : "";

            const { generateCreativeProposal } = await import("../services/agents/production/directorAgent");
            const proposal = await generateCreativeProposal(
                content.script || "",
                brandDNA,
            ...
            await db.update(projectContent)
                .set({ creativeProposal: JSON.stringify(proposal), proposalStatus: "pending_review" } as any)
                .where(eq(projectContent.projectId, input.projectId));

            return { success: true, proposal };
        }),

    approveCreativeProposal: publicProcedure
        .input(z.object({ projectId: z.number() }))
        .mutation(async ({ input, ctx }) => {
            if (!ctx.user) throw new Error("Unauthorized");
            const db = await getDb();
            ...
            await db.update(projectContent)
                .set({ proposalStatus: "approved" } as any)
                .where(eq(projectContent.projectId, input.projectId));

            return { success: true };
        }),

    generateTechnicalScript: publicProcedure
        .input(z.object({ projectId: z.number() }))
        .mutation(async ({ input, ctx }) => {
            if (!ctx.user) throw new Error("Unauthorized");
            const db = await getDb();
            ...
            if (!content || !content.castingValidated || !content.cineValidated || !content.pdValidated) {
                throw new Error("All departments must be validated before generating the technical script.");
            }
            ...
            const response = await invokeLLM({
                messages: [
                    {
                        role: "system",
                        content: "You are the lead Director. Compile all approved departmental outputs into a single cohesive Technical Script for the Prompt Engineer to use. Summarize Casting, Cinematography, and Production Design into one master document."
                    },
                    {
                        role: "user",
                        content: `Approved Script:\n${content.script}\n\nCreative Proposal:\n${content.creativeProposal}\n\nCasting:\n${content.castingApprovedOutput}\n\nCinematography:\n${content.cineApprovedOutput}\n\nProduction Design:\n${content.pdApprovedOutput}\n\nGenerate the Technical Script.`
                    }
                ]
            });

            const technicalScript = (response.choices[0]?.message.content as string) || "";

            await db.update(projectContent)
                .set({ technicalScript, technicalScriptStatus: "approved" } as any)
                .where(eq(projectContent.projectId, input.projectId));

            return { success: true, technicalScript };
        }),
    ...
    createScenes: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (db) {
                const contentResult = await db.select().from(projectContent).where(eq(projectContent.projectId, input.projectId)).limit(1);
                const content = contentResult[0];
                if (!content || (content as any).proposalStatus !== "approved") {
                    throw new Error("Creative Proposal must be approved before breaking down scenes.");
                }
            }
    ...
    getFullProductionLayout: publicProcedure
        ...
        .query(async ({ input }) => {
            ...
            const shotIds = shotList.map((s: Shot) => s.id);
            ...
            return sceneList.map((scene: Scene) => {
                const sceneShots = shotList.filter((s: Shot) => s.sceneId === scene.id).map((shot: Shot) => {
                    const latestGen = gens.find((g: Generation) => g.shotId === shot.id);
                    const sImg = sImages.find((img: StoryboardImage) => img.shotNumber === shot.order);
                    const shotAssignedActors = actorBindings.filter((a: { shotId: number }) => a.shotId === shot.id);
    ...
    generateShotImage: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            const promptInput = {
                ...
                aspect_ratio: "16:9" // Standardize for video synthesis
            };
            ...
            // 5. Generate Image (Use NanoBanana Pro for production quality)
            const generatedImageUrl = await generateStoryboardImage(
                cleanPrompt,
                "imagen-4.0-generate-001", // Default model
                scene.projectId,
    ...
    trainActor: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            if (!ctx.user) throw new Error("Unauthorized");
            const trainLoRA = async (): Promise<any> => { throw new Error("LoRA training disabled during Gemini migration"); };

            // Start training
            const training = await trainLoRA();
